...
function create_Table($connection, $result)
{
    ...
    for ($j = 0 ; $j < $rows ; ++$j)
    {
        ...
        //REFERENCIA Y DISENOS
        $table_row = $table_row . "<td>" . 
        "<form method='post' action='../finalizado/ref&disenos_Finalizado.php'> 
        ...
        //ORDEN
        if(CheckOrdernFinalizado($connection, $orden_ID))
        {
            $table_row = $table_row . "<td>" . 
            "<form method='post' action='../finalizado/list_Ordenes_Finalizado.php'> 
        ...
        "<form method='post' action='../editar/editar_producto_submition.php' enctype='multipart/form-data'>".
            <<<_END
                <input type="hidden" name="Orden_ID_Restablecer" value="$orden_ID" />
                <button type='submit' name='Restablecer_Producto_Status' value='$producto_ID'  onclick="return confirm('Seguro que restruar status al producto y orden?')">
                -</button>
            _END
...
function CheckOrdernFinalizado($connection, $Orden_ID)
{
